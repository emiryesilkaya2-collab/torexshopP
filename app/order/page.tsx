...
export default function OrderPage() {
  ...
  async function searchOrder(value?: string) {
    ...
    const res = await fetch(
      `/api/order?tracking=${encodeURIComponent(finalTracking)}`
    );
      ...
      <div className="mx-auto max-w-3xl">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-2 text-zinc-400">
                Takip numaranı girip teslimat durumunu izle
              </p>
            </div>

            <Link
              href="/"
              className="rounded-xl border border-red-500/40 px-4 py-2 font-bold hover:bg-red-950"
            >
              Ana Sayfa
            </Link>
          </div>
        ...
        {order && (
          <div className="mt-6 rounded-[2rem] border border-red-500/20 bg-black/70 p-6 shadow-2xl shadow-black/40">
              ...
              <div className="flex items-center gap-5">
                ...
                <div>
                  ...
                  <div className="mt-1 text-sm text-zinc-500">
                    Takip No {order.tracking_number || "Yok"}
                  </div>
            ...
            {order.status === "ready" && !order.vip_link && (
              <div className="mt-6 rounded-2xl border border-blue-500/30 bg-blue-950/20 p-5">
                <div className="text-lg font-black text-blue-400">
                  Sipariş hazır
                </div>

                <p className="mt-2 text-zinc-300">
                  VIP sunucu linki kısa süre içinde eklenecek
                </p>
              </div>
            )}
